    ...
    ILogger<OnboardingService> logger) : IHostedService
{
    ...
    public async Task ShowAsync(bool force = false)
    {
        ...
        try
        {
            var content = new StackPanel
            {
                ...
                Children =
                {
                    ...
                    new InfoBar
                    {
                        ...
                        Title = "非官方个人项目",
                        Message = "HL Education + 由个人开发者“伊烬遥”独立开发，不代表任何学校、教育集团或其他机构，亦不存在隶属、授权或合作关系。"
                    },
                    new InfoBar
                    {
                        IsOpen = true,
                        ...
                        Message = "启用后，HL Education + 将完全接管每天加载哪份课表，并以多周周期结果覆盖 ClassIsland 原本的每日课表选择。原有课表内容不会被删除；暂停 HL Education + 后会恢复接管前的课表状态。"
                    },
                    ...
                    CreateStep("5", "配置大周值日生", "值日生默认暂停；在独立的“大周值日生”设置页首次应用名单后会自动开启，并只在 HL Education + 课表运行时推进。"),
                    new TextBlock
                    {
                        Text = isFirstRun
                            ? "只有点击“我已了解，开始配置”后，HL Education + 才会开始接管课表。"
                            : "关闭本窗口不会更改当前启用状态。",
            ...
            logger.LogInformation("HL Education + 新手引导已关闭，结果：{Result}", result);
    ...
    private async Task WaitForMainWindowAsync(CancellationToken cancellationToken)
    {
        ...
        {
            logger.LogError(exception, "等待主窗口显示 HL Education + 新手引导时发生错误。");
        }
    }
    ...
    private async Task ShowSafelyAsync()
    {
        ...
        {
            logger.LogError(exception, "显示 HL Education + 新手引导失败。");
        }
    }
